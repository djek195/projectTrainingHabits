...
const page = {
  ...
  content: {
    ...
    habitDay: document.querySelector(".habit__day"),
    habitComment: document.querySelector(".habit__comment"),
  },
  popup: {
    index: document.getElementById("add-habit-popup"),
    addHabit: document.querySelector(".menu__add"),
    hidePopup: document.querySelector(".popup__close"),
...
function rerenderContent(activeHabit) {
  ...
  for (const index in activeHabit.days) {
    ...
    element.innerHTML = ` <div class="habit__day">День ${+index + 1}</div>
    <div class="habit__comment">${activeHabit.days[index].comment}</div>
    <button class="habit__delate" onclick = "deleteDay(${index})"><img src="images/delete.svg" alt="Видалити день ${
      +index + 1
    }"></button>`;
...
function addHabit(event) {
  ...
  let maxId = habits.reduce((acc, habit) =>
    acc > habit.id ? acc : habit.id, 0
    );
    console.log(maxId);
  habits.push({
    id: maxId + 1,
